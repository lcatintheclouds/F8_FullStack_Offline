// findIndex: Tìm index đầu tiên dựa vào điều kiện trong call back
...
// Xoá các phần tử trùng trong mảng users
// Yêu cầu: Dùng reduce
...
//  Output [5, 9]
newArr = arr1.reduce(function (prev, current) {
...
/* function newNumbers(arr) {
  return number.reduce((prev, current) => {
    return prev.concat(Array.isArray(current) ? newNumbers(current) : current);
  }, []);
}
...
//  Yêu cầu: Dùng reduce
//  Ý tuoeng
/* var chunkArray = numbers.reduce(function (prev, current, index) {
  if (index % size === 0) {
...
chunkArray.forEach(function (subArray, index) {
